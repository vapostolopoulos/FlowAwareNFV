...
class STLS1(object):
    """ Create both a flow stat latency stream and a non latency stream of UDP packet.
    ...
    def _create_stream (self):
        ...
        base_pkt = Ether() / IP(src = "16.0.0.1", dst = "48.0.0.1") / UDP(dport = 14, sport = 1025)
        pad = max(0, size - len(base_pkt)) * 'x'
        pkt = STLPktBuilder(pkt = base_pkt/pad)


        base_pkt2 = Ether() / IP(src = "16.0.0.1", dst = "48.0.0.1") / UDP(dport = 12, sport = 1025)
        pad2 = max(0, size - len(base_pkt2)) * 'x'
        pkt2 = STLPktBuilder(pkt = base_pkt2/pad2)

        return [STLStream(packet = pkt2,
